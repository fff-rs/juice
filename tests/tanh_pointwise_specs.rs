...
mod tanh_pointwise_spec_cuda{
    ...
    fn it_computes_correct_tanh_on_cuda_for_f32() {
        ...
        match backend.tanh_pointwise(&mut x) {
            Ok(_) => {
                x.sync(native.device()).unwrap();
    ...
    fn it_computes_correct_tanh_on_cuda_for_f32_plain() {
        ...
        match backend.tanh_pointwise_plain(&mut x) {
            Ok(_) => {
                x.sync(native.device()).unwrap();
                if let Some(mem) = x.get(native.device()).unwrap().as_native() {
                    assert_eq!(&[0.7615942f32, 0.7615942f32, 0.9640276f32], mem.as_slice::<f32>());
    ...
    fn it_computes_correct_tanh_grad_on_cuda_for_f32() {
        ...
        match backend.tanh_pointwise_grad(&mut x, &mut x_diff) {
            Ok(_) => {
                x_diff.sync(native.device()).unwrap();
    ...
    fn it_computes_correct_tanh_grad_on_cuda_for_f32_plain() {
        ...
        match backend.tanh_pointwise_grad_plain(&mut x, &mut x_diff) {
            Ok(_) => {
                x_diff.sync(native.device()).unwrap();
                if let Some(mem) = x_diff.get(native.device()).unwrap().as_native() {
                    assert_eq!(&[0f32, 0f32, -6f32], mem.as_slice::<f32>());
...
#[cfg(test)]
#[cfg(feature = "native")]
mod tanh_pointwise_spec_native {
