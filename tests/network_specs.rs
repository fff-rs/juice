...
mod network_spec {
    ...
    fn mnist_forward_backward() {
        ...
        // let conv_layer_cfg = ConvolutionConfig {
        //     num_output: 20,
        //     filter_shape: vec![5],
        //     padding: vec![0], // ?
        //     stride: vec![1],
        //     axis: None
        // };
        // let mut conv_cfg = LayerConfig::new("conv".to_owned(), LayerType::Convolution(conv_layer_cfg));
        // conv_cfg.bottoms.push("in".to_owned());
        // conv_cfg.tops.push("conv_out".to_owned());
        // cfg.layers.push(conv_cfg);
    ...
    #[test]
    #[ignore]
    // modeled after https://github.com/soumith/convnet-benchmarks/blob/c4dfa528cd7f2abd2e9abd91b294f91d01146c42/caffe/imagenet_winners/alexnet.prototxt
    fn alexnet_forward() {
        let _ = env_logger::init();
        let mut cfg = NetworkConfig::default();
        // Layer: data
        ...
        let mut network = Network::from_config(backend.clone(), &cfg);
        let loss = &mut 0f32;
        let inp = Blob::from_data(SharedTensor::<f32>::new(backend.device(), &vec![128, 3, 224, 224]).unwrap());
        // let label = Blob::from_data(SharedTensor::<f32>::new(native_backend.device(), &vec![1, 1, 10]).unwrap());
    ...
    #[test]
    // modeled after https://github.com/soumith/convnet-benchmarks/blob/c4dfa528cd7f2abd2e9abd91b294f91d01146c42/caffe/imagenet_winners/alexnet.prototxt
    // smaller version to fit into 2GB of GPU memory
    // - reduced the input dimensions from 224 to 112
    // - halfed all the convolution layers output dimensions
    fn small_alexnet_forward() {
        let _ = env_logger::init();
        let mut cfg = NetworkConfig::default();
        // Layer: data
        cfg.inputs.push("data".to_owned());
