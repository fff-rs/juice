...
impl<B: IBackend + conn::Sigmoid<f32>> Layer<B> for Sigmoid {
    fn compute_output(&self, backend: &B, context: &mut Context) -> Result<(), LayerError> {
        ...
        backend.sigmoid(&input.borrow(), &mut output.borrow_mut())?;
        Ok(())
    }
...
mod tests {
    ...
    fn compute() {
        ...
        assert_tensor_eq(
            &result.output,
            &create_tensor_2d([[0.7310586, 0.11920292], [0.047425874, 0.98201376]]),
        );
    }
    ...
    fn compute_gradients() {
        ...
        assert_tensor_eq(
            ...
            &create_tensor_2d([[0.078644775, 0.031498075], [0.004517666, 0.0035325468]]),
        );
    }
}
