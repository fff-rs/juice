...
impl<B: IBackend + Axpby<f32> + Copy<f32>> Layer<B> for MeanSquaredError {
    ...
    fn compute_gradients(&self, backend: &B, context: &mut Context) -> Result<(), LayerError> {
        ...
        let predictions_data = predictions_ref.read(native.device())?.as_slice::<f32>();

        let labels_ref = labels.borrow();
        let labels_data = labels_ref.read(native.device())?.as_slice::<f32>();

        let mut input_gradient_ref = input_gradient.borrow_mut();
        let input_gradient_data = input_gradient_ref.write_only(native.device())?.as_mut_slice::<f32>();
