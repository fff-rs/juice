...
use co::{DeviceType,SharedTensor};

/// Returns cuDNN ready memory pointer from a SharedTensor.
pub unsafe fn receive_memory_ptr<T>(x: &SharedTensor<T>, device: &DeviceType) -> Result<*const ::libc::c_void, ::co::plugin::Error> {
...
pub unsafe fn receive_memory_ptr_mut<T>(x: &mut SharedTensor<T>, device: &DeviceType) -> Result<*mut ::libc::c_void, ::co::plugin::Error> {
    ...
    ($t:ty, $b:ty) => (
        ...
        fn convolution_plain(
            ...
            Ok(try!(match CUDNN.convolution_forward(
                ...
                Ok(_) => Ok(()),
                Err(_) => {
                    Err(::co::plugin::Error::Operation("Unable to execute CUDA cuDNN convolution forward."))
                }
            }))
        }
        ...
        fn convolution_grad_filter_plain(
            ...
            Ok(try!(match CUDNN.convolution_backward_filter(
                ...
                Err(err) => {
                    println!("{:?}", err);
                    Err(::co::plugin::Error::Operation("Unable to execute CUDA cuDNN convolution backward w.r.t. filter."))
                }
            }))
        }
        ...
        fn convolution_grad_data_plain(
            ...
            Ok(try!(match CUDNN.convolution_backward_data(
                ...
                Err(err) => {
                    println!("{:?}", err);
                    Err(::co::plugin::Error::Operation("Unable to execute CUDA cuDNN convolution backward w.r.t data."))
                }
            }))
        }
    )
