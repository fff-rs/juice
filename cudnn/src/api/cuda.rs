...
impl API {
    ...
    unsafe fn ffi_cuda_allocate_device_memory(bytes: usize) -> Result<*mut ::libc::c_void, Error> {
        ...
        match cudaMalloc(&mut ptr, bytes) {
            ...
            cudaError_t::cudaErrorMemoryAllocation => Err(Error::AllocFailed("Unable to allocate CUDA device memory.")),
            _ => Err(Error::Unknown("Unable to allocate CUDA device memory for unknown reasons."))
        }
    }
    ...
    unsafe fn ffi_cuda_free_device_memory(ptr: *mut ::libc::c_void) -> Result<(), Error> {
        match cudaFree(ptr) {
             ...
             // TODO, more error enums sigh
     	     cudaError_t::cudaErrorInvalidDevicePointer => Err(Error::Unknown("Unable to free the CUDA device memory.")),
     	     cudaError_t::cudaErrorInitializationError => Err(Error::Unknown("CUDA Driver/Runtime API not initialized.")),
