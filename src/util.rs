...
pub trait LayerOps<F: Float> : conn::Convolution<F> + conn::Pooling<F> + conn::Relu<F> + conn::Sigmoid<F> + conn::Softmax<F>
                             + Gemm<F> {}

impl<T: conn::Convolution<f32> + conn::Pooling<f32> + conn::Relu<f32> + conn::Sigmoid<f32> + conn::Softmax<f32>
